...
function SearchBox({
  ...
  const results = useMemo((): SearchResult[] => {
    ...
    graph.forEachNode((id, attrs) => {
      ...
      if (a.parent_external_id) return // skip children — only groups and standalones
          ...
          {results.map((r) => (
            <li key={r.id} className="search-group">
              <div className="search-row">
                <span onMouseDown={() => { onSelect(r.id); setQ(''); setOpen(false) }}>
                  <span className={`type-dot ${r.type}`} />
                  <span className="type-label">{tNodeType(r.type, lang)}</span>
                  {r.name}
                </span>
              </div>
            </li>
...
export default function App() {
  ...
  }, [rootNodeId, extraNodeIds, fadedNodeIds, activeEdgeIds, hiddenNodeIds, lockedNodes, manualEdges])
  ...
  const visibleNodeIds = useMemo(() => {
    ...
    return base
  }, [graph, rootNodeId, activeEdgeIds, hiddenNodeIds, extraNodeIds])
  ...
  // Add a standalone node to the canvas without resetting existing content
  ...
  const autoLayout = useCallback(() => {
    ...
    for (const id of g.nodes()) {
      if (lockedNodes.has(id)) continue // respect manually-dragged positions
  ...
  }, [graph, rootNodeId, activeEdgeIds, selectedNodeId, extraNodeIds])
  ...
  // fires every drag frame — keeps ref in sync with RF without triggering re-renders
  ...
  const handleEdgeClick = useCallback((edgeId: string) => {
    ...
    if (activeEdgeIds.has(edgeId)) {
      ...
      // mirror onActivateEdge cascade: in simples mode, auto-follow success out-edge
  ...
  const { cyNodes, cyEdges } = useMemo(() => {
    ...
    // ── Tudo: dump all nodes in a type-grouped grid ───────────────────────
    if (viewMode === 'tudo') {
      const typeOrder: Record<string, number> = { position: 0, submission: 1, transition: 2 }
    ...
    // ── Simples: transitions collapsed into labeled edges ─────────────────
    if (viewMode === 'simples') {
    ...
    // ── Completo: standard nodes + edges, group parents always show mini-cards ─

    const hiddenChildren = new Set<string>()
    for (const nodeId of visibleNodeIds) {
      for (const cid of (childrenByParentId.get(nodeId) ?? [])) {
        if (!childrenByParentId.has(cid)) hiddenChildren.add(cid) // skip nested group parents
      }
    }
        ...
        childrenSummary: isGroup
          ? childIds!
              ...
              .filter((cid) => {
                ...
                return graph.edges(cid).some((eid) => {
                  if (activeEdgeIds.has(eid)) return true
                  const other = graph.source(eid) === cid ? graph.target(eid) : graph.source(eid)
                  return visibleNodeIds.has(other)
                })
    ...
    const addEdge = (edgeId: string, state: 'active' | 'latent') => {
      ...
      const srcIsTransition = graph.hasNode(rawSrc) && graph.getNodeAttributes(rawSrc).node_type === 'transition'
      if (ea.result_type === 'failure' && !srcIsTransition) return
      ...
      let sourceHandle = hiddenChildren.has(rawSrc) ? `child-${rawSrc}-out` : undefined
      let targetHandle = hiddenChildren.has(rawTgt) ? `child-${rawTgt}-in` : undefined
      if (!sourceHandle && srcIsTransition) {
        sourceHandle = ea.result_type === 'failure' ? 'falha' : 'sucesso'
      }
      if (!targetHandle && graph.hasNode(rawTgt) && graph.getNodeAttributes(rawTgt).node_type === 'transition') {
        targetHandle = 'antes'
      }
    ...
    childrenByParentId, parentByChildId, showOnlyActiveChildren, layoutVersion,
  ])

  // ── merge manual edges into cyEdges ────────────────────────────────────
